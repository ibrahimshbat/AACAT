...
public class ControlButtons extends JPanel implements ActionListener {
	...
	/**
	 * This method is used to make the Next button inactive after after finishing inserting or 
	 * finding a number in the AVL tree data structure and make other buttons active
	 */
	 ...
	 * So, if n is 0 the insert operation will run and if n is 1 the find operation
	 * will run.
	 * @param n
	 * 			- int
	 * @param nums
	...
	public void action(int n, ArrayList<Integer> nums) {
		...
		switch (n) {
		case 0:
			// check if the user pressed the Insert button without typing a number 
			if (nums.size() == 0) {
			...
			break;
		case 1:
			// find only one number, which is the fist number typed in the text field
